...
describe('Fetch teacher lessons metrics', () => {
  beforeEach(() => {
    vi.useFakeTimers()
  })
  ...
  it('Should fetch the teacher lessons metrics', async () => {
    ...
    const response = await request(app.server)
      .get(`/lessons/teachers/metrics`)
      .set('Authorization', `Bearer ${token}`)
      .send()

    expect(response.status).toEqual(200)
    expect(response.body.lessonsCount).toEqual(2)
  })
})
